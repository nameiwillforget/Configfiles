...
(helm-define-key-with-subkeys global-map (kbd "C-c t") ?n 'helm-cycle-resume)
...
(with-eval-after-load 'helm-dictionary ; Its autoloads are already loaded.
  (setq helm-dictionary-database
        '(("de-en" . "/home/alex/.emacs.d/helm-dictionaries/de-en.txt")))
  (setq helm-dictionary-online-dicts
	  ...
	  ("en.wikipedia.org" . "http://en.wikipedia.org/wiki/%s")
          ("de.wikipedia.org" . "http://de.wikipedia.org/wiki/%s"))
))
...
(global-set-key (kbd "C-c <f4>")                    'helm-mark-ring)
...
(define-key global-map (kbd "C-c n r")               'helm-dictionary)
